...
namespace Noxico
{
    public static class DungeonGenerator
    {
            ...
            */
            ...
            for (var i = 0; i < levels.Count; i++)
            {
                for (var j = 0; j < levels[i].Count; j++)
                {
                    ...
                    board.ID = string.Format("Dng_{0}_{1}{2}", DungeonGeneratorEntranceBoardNum, i + 1,
                        (char) ('A' + j));
                    ...
                    if (board == entranceBoard)
                    {
                        ...
                        board.SetTile(exit.YPosition, exit.XPosition,
                            "dungeonExit"); //board.SetTile(exit.YPosition, exit.XPosition, '<', Color.Silver, Color.Black);
                    }
                }
            }
            ...
            for (var i = 0; i < levels.Count; i++)
            {
                ...
                boardHere.SetTile(here.YPosition, here.XPosition,
                    up
                        ? "dungeonUpstairs"
                        : "dungeonDownstairs"); //boardHere.SetTile(here.YPosition, here.XPosition, up ? '<' : '>', Color.Gray, Color.Black);
                boardThere.SetTile(there.YPosition, there.XPosition,
                    up
                        ? "dungeonDownstairs"
                        : "dungeonUpstairs"); //boardThere.SetTile(there.YPosition, there.XPosition, !up ? '<' : '>', Color.Gray, Color.Black);
            ...
            for (var i = 0; i < levels.Count; i++)
            {
                for (var j = 0; j < levels[i].Count - 1; j++)
                {
                    ...
                    there.TargetWarpID = here.ID;
                    boardHere.SetTile(here.YPosition, here.XPosition,
                        "dungeonSideexit"); //boardHere.SetTile(here.YPosition, here.XPosition, '\x2261', Color.Gray, Color.Black);
                    boardThere.SetTile(there.YPosition, there.XPosition,
                        "dungeonSideexit"); //boardThere.SetTile(there.YPosition, there.XPosition, '\x2261', Color.Gray, Color.Black);
            ...
            while (true)
            {
                ...
                if (goalBoard.IsSolid(treasureY, treasureX))
                    continue;
                //2013-03-07: prevent placing warps on same tile as clutter
                if (goalBoard.Entities.FirstOrDefault(e => e.XPosition == treasureX && e.YPosition == treasureY) !=
                    null)
                {
                ...
                if (goalBoard.IsSolid(treasureY, treasureX + 1))
                    sides++;
                if (sides < 3 && sides > 1 &&
                    goalBoard.Warps.FirstOrDefault(w => w.XPosition == treasureX && w.YPosition == treasureY) == null)
                    break;
            }
        ...
        public static List<Token> GetLoots(string target, string type, Dictionary<string, string> filters = null)
        {
            ...
            foreach (var potentialSet in lootDoc.Where(t =>
                         t.Name == "lootset" && t.GetToken("target").Text == target && t.GetToken("type").Text == type))
            {
                ...
                if (setsFilters != null && setsFilters.Tokens.Count > 0)
                {
                    ...
                    foreach (var f in setsFilters.Tokens)
                    {
                        var key = f.Name;
                        var value = f.Text;
                        if (filters.ContainsKey(key) && ((value[0] != '!' && filters[key] != value) ||
                                                         (value[0] == '!' && filters[key] == value.Substring(1))))
                        {
                            isOkay = false;
                            break;
                        }
        ...
        public static List<Token> GetRandomLoot(string target, string type, Dictionary<string, string> filters = null)
        {
            ...
            Func<string, Token> parseOption = new Func<string, Token>(option =>
            {
                if (option[0] == '@')
                {
                        ...
                        foreach (var knownItem in NoxicoGame.KnownItems.Where(i => !i.HasToken("unique")))
                        {
                            ...
                            if (includeThis)
                            {
                                ...
                                if (knownItem.HasToken("colored"))
                                    newPoss.AddToken("color", 0,
                                        colors[color == -1 ? Random.Next(colors.Count) : color]);
                    ...
                    {
                        //simple token check
                        var items = NoxicoGame.KnownItems.Where(i => i.HasToken(option) && !i.HasToken("unique"))
                            .ToList();
                        if (items.Count > 0)
                        {
    ...
    public class PerlinNoise
    {
        ...
        public float GetRandomHeight(float x, float y, float maxHeight, float frequency, float amplitude,
    ...
    public class WorldMapGenerator
    {
            ...
            for (var row = 0; row < rows; row++)
            {
                for (var col = 0; col < cols; col++)
                {
                    ...
                    for (var i = 0; i < BiomeData.Biomes.Count; i++)
                    {
                        ...
                        if (b.Realm == Realm && t >= b.Rect.Left && t <= b.Rect.Right && p >= b.Rect.Top &&
                            p <= b.Rect.Bottom)
                        {
                            map[row, col] = (byte) i;
                            continue;
        ...
        public void GenerateWorldMap(Realms realm, Action<string, int, int> setStatus)
        {
            ...
            DetailedMap = bmp;
            Program.WriteLine("{0} -- Draw board bitmap ({1} by {2} px/tiles).", stopwatch.Elapsed, bmpWidth,
                bmpHeight);
            ...
            {
                setStatus(i18n.Format("worldgen_markingtowns", realm), bRow,
                    MapSizeY); //"Marking possible town locations..."
