...
namespace Noxico
{
    ...
    public partial class Character
    {
        public List<Token> GetTargetedMorphDeltas(string targetPlan, Gender targetGender)
        {
            ...
            var meta = new[]
                {"playable", "culture", "namegen", "bestiary", "femalesmaller", "costume", "_either", "items"};
            ...
            foreach (var trivialKind in trivialKinds)
            {
                ...
                {
                    var options = thenToken.Text.Substring(thenToken.Text.IndexOf("of ") + 3).Split(',')
...

            {
                ...
                {
                    var options = skinTypeThen.Substring(skinTypeThen.IndexOf("of ") + 3).Split(',')
                        .Select(x => x.Trim()).ToArray();
                ...
                {
                    var options = skinColorThen.Substring(skinColorThen.IndexOf("of ") + 3).Split(',')
                        .Select(x => x.Trim()).ToArray();
                ...
                if (skinTypeNow != skinTypeThen)
                {
                    ...
                    if (skinTypeThen != "slime")
                    {
                        ...
                        {
                            var options = legsThen.Substring(legsThen.IndexOf("of ") + 3).Split(',')
                                .Select(x => x.Trim()).ToArray();
...

            {
                ...
                if (hairThen != null && hairThen.GetToken("length").Value > 0)
                {
                    ...
                    {
                        var options = hairColorThen.Substring(hairColorThen.IndexOf("of ") + 3).Split(',')
                            .Select(x => x.Trim()).ToArray();
...

            {
                ...
                {
                    var options = tailThen.Text.Substring(tailThen.Text.IndexOf("of ") + 3).Split(',')
                        .Select(x => x.Trim()).ToArray();
...

            {
                ...
                {
                    var options = wingsThen.Text.Substring(wingsThen.Text.IndexOf("of ") + 3).Split(',')
                        .Select(x => x.Trim()).ToArray();
                ...
                foreach (var multiLeg in multiLegs)
                {
                    if (target.HasToken(multiLeg) && !this.HasToken(multiLeg))
                    {
                        ...
                        switch (multiLeg)
                        {
                            case "taur":
                                change.AddToken("$",
                                    "[views] lower body turns into a quadrupedal, centauroid configuration");
                                break;
                            case "quadruped":
                                change.AddToken("$", "[views] entire body turns into a quadrupedal setup");
                                break;
                        }
            ...
            //No need to *add* breasts -- you always have them even if flat.
            {
                ...
                {
                    change = new Token("breasts/nipples/size",
                        sourceBreasts.GetToken("nipples").GetToken("size").Value + (nipplesGrowOrShrink * 0.25f));
                ...
                else
                {
                    ...
                    else
                    {
                        if (loosenOrTighten != 0)
                            change = new Token("vagina/looseness",
                                sourceVagina.GetToken("looseness").Value + loosenOrTighten);
                        if (wettenOrDry != 0)
                        {
                            if (change == null)
                                change = new Token("vagina/wetness",
                                    sourceVagina.GetToken("wetness").Value + (wettenOrDry * 0.25f));
                            else
                                change.AddToken("vagina/wetness",
                                    sourceVagina.GetToken("wetness").Value + (wettenOrDry * 0.25f));
                        }
            ...
            if (targetGender == Noxico.Gender.Male || targetGender == Noxico.Gender.Herm)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (change == null)
                                change = new Token("penis/thickness",
                                    sourceDick.GetToken("thickness").Value + (thickOrThin * 0.25f));
                            else
                                change.AddToken("penis/thickness",
                                    sourceDick.GetToken("thickness").Value + (thickOrThin * 0.25f));
                        }
            ...
            if (targetGender == Noxico.Gender.Male || targetGender == Noxico.Gender.Herm)
            {
                ...
                else
                {
                    ...
                    {
                        if (change == null)
                            change = new Token("balls/size",
                                sourceBalls.GetToken("size").Value + (growOrShrink * 0.25f));
                        else
        ...
        public List<Token> GetWildMorphDeltas(float intensity, Mutations mutation)
        {
            if (mutation == Mutations.Random)
                mutation = (Mutations) Random.Next(Enum.GetNames(typeof(Mutations)).Length -
                                                   1); //subtract one from the length to account for Random = -1
            ...
            switch (mutation)
            {
                ...
                case Mutations.AddBreast:
                    if (!this.HasToken("breasts"))
                    {
                        ...
                        if (nips != null)
                        {
                            ...
                            foreach (var nipTok in nips.Tokens)
                            {
                                ...
                                if (!nipTok.Text.IsBlank())
                                    change.AddToken("breasts/nipples/" + nipTok.Name, nipTok.Text);
                            }
                        }
                    ...
                    else
                    {
                        ...
                        change.AddToken("$",
                            i18n.Format("morph_gainmorebreasts", (this.Path("breasts/amount").Value + 1).Ordinal()));
                        possibleChanges.Add(change);
                    }
                ...
                case Mutations.RemoveOddLegs:
                {
                    foreach (var snakeSlime in new[] {"slimeblob", "snaketail"})
                    {
                        if (this.HasToken(snakeSlime))
                        {
                            ...
                            change.AddToken("$",
                                i18n.Format("morph_lose_x_and_gain_y_legs", i18n.GetString("morph_lose" + snakeSlime),
                                    newLegs));
                ...
                case Mutations.AddPenis:
                    ...
                    else
                    {
                        ...
                        {
                            var change = new Token("penis/length",
                                this.Path("penis/length").Value + (float) Random.NextDouble() * intensity + 2f);
                            change.AddToken("penis/thickness",
                                this.Path("penis/thickness").Value + (float) Random.NextDouble() * (intensity / 3));
                ...
                case Mutations.AddVagina:
                    ...
                    else
                    {
                        ...
                        {
                            var change = new Token("vagina/looseness",
                                this.Path("vagina/looseness").Value + (float) Random.NextDouble() * intensity);
                            change.AddToken("vagina/wetness",
                                this.Path("vagina/wetness").Value + (float) Random.NextDouble() * intensity);
        ...
        public string GetMorphFeedback(List<string> feedbacks)
        {
            ...
            {
                if (isStart[i])
                    feedbackBuilder.Append(feedbacks[i].Replace("[views]", "[Yourornames]")
                        .Replace("[view]", "[Youorname]"));
                else
        ...
        public string Morph(string targetPlan, Gender targetGender = Gender.Invisible)
        {
            ...
            SpeechFilter =
                null; //invalidate here -- we don't necessarily have this character speak right away and need to adjust for new impediments.
