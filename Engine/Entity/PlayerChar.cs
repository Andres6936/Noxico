...
namespace Noxico
{
    public class Player : BoardChar
    {
        ...
        public int Lives
        {
            get { return Character.HasToken("lives") ? (int) Character.GetToken("lives").Value : 0; }
            set
            {
        ...
        {
            var warp = ParentBoard.Warps.Find(w => /* !String.IsNullOrEmpty(w.TargetBoard) && */
                w.XPosition == XPosition && w.YPosition == YPosition);
            if (warp != null)
            {
                ...
                var targetBoard =
                    game.GetBoard(warp
                        .TargetBoard); //game.Boards[warp.TargetBoard]; //.Find(b => b.ID == warp.TargetBoard);
        ...
        public override void Move(Direction targetDirection, SolidityCheck check = SolidityCheck.Walker)
        {
            ...
            else if (ly == ParentBoard.Height - 1 && targetDirection == Direction.South &&
                     this.ParentBoard.ToSouth > -1)
            {
            ...
            foreach (var entity in ParentBoard.Entities.Where(x => x.XPosition == newX && x.YPosition == newY))
            {
                if (entity.Blocking)
                {
                    ...
                    if (entity is BoardChar)
                    {
                        ...
                        if (bc.Character.HasToken("hostile") || (bc.Character.HasToken("teambehavior") &&
                                                                 bc.Character.DecideTeamBehavior(Character,
                                                                     TeamBehaviorClass.Attacking) !=
                        ...
                        NoxicoGame.AddMessage(
                            i18n.Format("youdisplacex", bc.Character.GetKnownName(false, false, true)),
                            bc.GetEffectiveColor());
            ...
            if (Character.HasToken("squishy") ||
                (Character.Path("skin/type") != null && Character.Path("skin/type").Text == "slime"))
            ...
            else if (ParentBoard.Entities.OfType<DroppedItem>()
                         .FirstOrDefault(c => c.XPosition == XPosition && c.YPosition == YPosition) != null)
                NoxicoGame.ContextMessage = i18n.GetString("context_droppeditem");
            else if (ParentBoard.Entities.OfType<Container>()
                         .FirstOrDefault(c => c.XPosition == XPosition && c.YPosition == YPosition) != null)
            {
                if (ParentBoard.Entities.OfType<Container>()
                    .FirstOrDefault(c => c.XPosition == XPosition && c.YPosition == YPosition).Token.HasToken("corpse"))
            ...
            else if (ParentBoard.Entities.OfType<Clutter>().FirstOrDefault(c =>
                         c.XPosition == XPosition && c.YPosition == YPosition && c.DBRole == "bed") != null)
                NoxicoGame.ContextMessage = i18n.GetString("context_bed");

            if (NoxicoGame.ContextMessage != null)
                NoxicoGame.ContextMessage = Toolkit.TranslateKey(KeyBinding.Activate, false, false) + " - " +
                                            NoxicoGame.ContextMessage;
        }
        ...
        public void QuickFire(Direction targetDirection)
        {
            ...
            Func<int, int, bool> gotHit = (xPos, yPos) =>
            {
                ...
                var hit = this.ParentBoard.Entities.OfType<BoardChar>()
                    .FirstOrDefault(e => e.XPosition == x && e.YPosition == y);
                if (hit != null)
                {
                    ...
                    FireLine(weapon.Path("effect"), x, y);
                    NoxicoGame.AddMessage(i18n.Format("youhitxfory", hit.Character.GetKnownName(false, false, true),
                        damage, i18n.Pluralize("point", (int) Math.Ceiling(damage))));
            ...
            {
                for (x++;
                     x < this.ParentBoard.Width && distance < range;
                     x++, distance++) //TODO: confirm if boardspace or screenspace
                    if (gotHit(x, y))
                        break;
            }
            ...
            {
                for (y++;
                     y < this.ParentBoard.Height && distance < range;
                     y++, distance++) //TODO: confirm if boardspace or screenspace
                    if (gotHit(x, y))
                        break;
            }
        ...
        public override void Update()
        {
            ...
            //RED
            if ((NoxicoGame.IsKeyDown(KeyBinding.Activate) || Vista.Triggers == XInputButtons.B) && !helpless &&
                !flying)
            {
                NoxicoGame.ClearKeys();
                ...
                var container = ParentBoard.Entities.OfType<Container>()
                    .FirstOrDefault(c => c.XPosition == XPosition && c.YPosition == YPosition);
                if (container != null)
                {
                    NoxicoGame.ClearKeys();
                ...
                var bed = ParentBoard.Entities.OfType<Clutter>().FirstOrDefault(c =>
                    c.XPosition == XPosition && c.YPosition == YPosition && c.DBRole == "bed");
                if (bed != null)
                {
                    Character.Posture = Posture.Prone;
                    var prompt = "It's " + NoxicoGame.InGameTime.ToShortTimeString() + ", " +
                                 NoxicoGame.InGameTime.ToLongDateString() + ". Sleep for how long?";
            ...
            {
                if (NoxicoGame.IsKeyDown(KeyBinding.Left) || NoxicoGame.IsKeyDown(KeyBinding.Right) ||
                    NoxicoGame.IsKeyDown(KeyBinding.Up) ||
                    NoxicoGame.IsKeyDown(KeyBinding
                        .Down)) //(NoxicoGame.KeyMap[(int)Keys.Left] || NoxicoGame.KeyMap[(int)Keys.Right] || NoxicoGame.KeyMap[(int)Keys.Up] || NoxicoGame.KeyMap[(int)Keys.Down])
                {
                    AutoTravelling = false;
                    return;
                }
        ...
        public override bool Hurt(float damage, string cause, object aggressor, bool finishable = false,
            ...
            if (dead)
            {
                if (aggressor != null && aggressor is BoardChar)
                {
                    ...
                    if (aggChar.HasToken("stolenfrom"))
                    {
                        ...
                        var stolenGoods = myItems
                            .Where(t => t.HasToken("owner") && t.GetToken("owner").Text == aggChar.ID).ToList();
        ...
        public new void AimShot(Entity target)
        {
            ...
            if (aimSuccess)
            {
                if (target is BoardChar)
                {
                    ...
                    NoxicoGame.AddMessage(i18n.Format("youhitxfory", hit.Character.GetKnownName(false, false, true),
                        damage, i18n.Pluralize("point", (int) damage)));
                    hit.Hurt(damage, "death_shot", this, false);
                }
