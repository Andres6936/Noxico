...
namespace Noxico
{
    ...
    public class Entity
    {
        ...
        public virtual void Move(Direction targetDirection, SolidityCheck check)
        {
            ...
            if (XPosition >= 0 && YPosition >= 0 && XPosition < this.ParentBoard.Width &&
                YPosition < this.ParentBoard.Height)
        ...
        public virtual bool CanSee(Entity other)
        {
            foreach (var point in Toolkit.Line(XPosition, YPosition, other.XPosition, other.YPosition))
                ...
                if (ParentBoard.IsSolid(point.Y, point.X, SolidityCheck.Projectile) &&
                    ParentBoard.IsLit(point.Y, point.X))
                    return false;
            return true;
        }
    }
    ...
    public class Clutter : Entity
    {
        ...
        public Clutter(char character, Color foreColor, Color backColor, bool blocking = false, string name = "thing",
            ...
            if (knownThing != null)
            {
                ...
                if (knownThing.HasToken("color"))
                {
                    ...
                    thing.BackgroundColor =
                        thing.ForegroundColor
                            .Darken(); //TileDefinition.Find(parentBoardHack.Tilemap[e.XPosition, e.YPosition].Index).Background;
                }

                if (knownThing.HasToken("background"))
                {
                    if (knownThing.GetToken("background").Text == "inherit")
                        thing.BackgroundColor = TileDefinition.Find((thing.ParentBoard ?? ParentBoardHack)
                            .Tilemap[thing.XPosition, thing.YPosition].Index).Background;
                    else
    ...
    public class DroppedItem : Entity
    {
        ...
        {
            return new List<DroppedItem>(board.Entities.OfType<DroppedItem>()
                .Where(drop => drop.XPosition == x && drop.YPosition == y));
        }
        ...
        public static void PickItemsFrom(List<DroppedItem> items)
        {
            ...
            ActionList.Show(i18n.GetString("action_pickup_window") /* "Pick up..." */, items[0].XPosition,
                items[0].YPosition, itemDict,
                () =>
                {
                    ...
                    else
                    {
                        ...
                        player.Energy -= 1000;
                        NoxicoGame.AddMessage(i18n.Format("youpickup_x",
                            item.ToString(token, true))); //, drop.ForegroundColor);
                    }
    ...
    public class Container : Entity
    {
        ...
        public string Name
        {
            get { return Token.Text.IsBlank("container", Token.Text); }
    ...
    public class Door : Entity
    {
        ...
        public void UpdateMapSolidity()
        {
            if (ParentBoard == null)
                return;
            ParentBoard.Tilemap[XPosition, YPosition].Definition =
                TileDefinition.Find(closed ? "doorwayClosed" : "doorwayOpened");
        }
