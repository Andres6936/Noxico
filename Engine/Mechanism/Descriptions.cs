...
namespace Noxico
{
    public static class Descriptions
    {
        ...
        public static string Item(InventoryItem knownItem, Token token, string article = "", bool withColor = false)
        {
            if (knownItem == null)
                return null;
            var name = (token != null && token.HasToken("unidentified") && !knownItem.UnknownName.IsBlank())
                ? knownItem.UnknownName
                : knownItem.Name;
        ...
        public static string HairLength(Token hairToken)
        {
            if (hairToken == null)
                return "glitch";
            return GetSizeDescription("hair/length",
                hairToken.HasToken("length") ? hairToken.GetToken("length").Value : 0f);
        }
        ...
        public static string NippleSize(Token nipplesToken)
        {
            if (nipplesToken == null)
                return "glitch";
            return GetSizeDescription("breasts/nipples",
                nipplesToken.HasToken("size") ? nipplesToken.GetToken("size").Value : 0.25f);
        }
        ...
        {
            var request = descTable.Path("feet/" + (legsToken == null ? "default" : legsToken.Text)) ??
                          descTable.Path("feet/default");
            return request.Text.Pluralize(plural ? 2 : 1);
        }
