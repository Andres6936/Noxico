...
namespace Noxico
{
        ...
        public static bool LimitsOkay(Character[] actors, Token c)
        {
            ...
            if (env.GetClothing == null)
                env.GetClothing = new Func<Character, string, int, bool>((a, clothClass, s) =>
                {
                    ...
                    var slots = clothClass == "top" ? new[] {"cloak", "jacket", "shirt", "undershirt"} :
                        clothClass == "bottom" ? new[] {"pants", "underpants"} : null;
                    if (slots == null)
                    {
    ...
    public partial class Character
    {
        ...
        public bool Disrobe(string clothClass, bool tear)
        {
            ...
            else
            {
                foreach (var slot in slots)
                {
                    ...
                    if (cloth != null && cloth.CarriedToken.ContainsKey(this.ID) &&
                        cloth.CarriedToken[this.ID].HasToken("torn"))
                        continue; //scan along
                    if (cloth != null)
                        break;
                }
        ...
        public bool EnsureSexPartner()
        {
            ...
            if (sexPartner == null)
            {
                var s = this.BoardChar.ParentBoard.Entities.OfType<BoardChar>()
                    .FirstOrDefault(b => b.Character.ID == havingSex.Text);
                if (s != null)
            ...
            {
                Program.WriteLine("SEX: {0} is supposed to be having sex with {1} but that character isn't here.",
                    this.ID, sexPartner.Name);
