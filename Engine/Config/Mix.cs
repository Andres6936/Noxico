    ...
    public static class Mix
    {
                    ...
                    while (true)
                    {
                        ...
                        if (header[0] != 'P' || header[1] != 'K' || header[2] != 3 || header[3] != 4)
                        {
                            if (header[2] == 1 && header[3] == 2) //reached the Central Directory
                                break;
                            throw new FileLoadException(string.Format("MIX file '{0}' has an incorrect header.",
                                mixFile));
                        }
                        ...
                        var uncompressedSize =
                            (moto[3] << 24) | (moto[2] << 16) | (moto[1] << 8) | moto[0]; //0x00000197
        ...
        public static byte[] GetBytes(string fileName)
        {
            ...
            using (var mStream = new BinaryReader(File.Open(entry.MixFile, FileMode.Open)))
            {
                ...
                else
                {
                    ...
                    var decompressor =
                        new System.IO.Compression.DeflateStream(cStream,
        ...
        public static void SpreadEm()
        {
            ...
            foreach (var entry in FileList.Values)
            {
                if (entry.Length == 0)
                {
                    Program.WriteLine("* Bogus entry with zero length: \"{0}\", offset {1}", entry.Filename,
                        entry.Offset);
                    continue;
                }

                var targetPath = Path.Combine("data", /* entry.MixFile.Remove(entry.MixFile.Length - 4),*/
                    entry.Filename);
                ...
                using (var mStream = new BinaryReader(File.Open(entry.MixFile, FileMode.Open)))
                {
                    ...
                    {
                        var cStream = new MemoryStream(mStream.ReadBytes(entry.Length));
                        var decompressor = new System.IO.Compression.GZipStream(cStream,
                            System.IO.Compression.CompressionMode.Decompress);
                        var outStream = new MemoryStream();
                        var buffer = new byte[1024];
                        var recieved = 1;
