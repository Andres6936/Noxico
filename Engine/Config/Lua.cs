        ...
        public static void Ascertain(dynamic env = null)
        {
            ...
            env.print = new Action<object[]>(x =>
            {
                if (x.Length == 1 && x[0] is LuaTable)
                    Program.WriteLine("Table: {{ " + ((LuaTable) x[0]).Values.Select(v =>
                        string.Format("{0} = {1}", v.Key,
                            v.Value is string ? '\"' + v.Value.ToString() + '\"' : v.Value)).Join() + " }}");
                else
        ...
        public static LuaResult Run(string block, LuaGlobal env = null)
        {
            ...
            else
            {
                ...
                catch (LuaException pax)
                {
                    ...
                    throw new Exception(
                        "Lua parse error while trying to run this chunk:" + System.Environment.NewLine +
                        PrepareParseError(block, pax.Line, pax.Column) + System.Environment.NewLine + pax.Message, pax);
                }
            }
