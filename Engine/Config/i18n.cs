    ...
    public static class i18n
    {
            ...
            foreach (var test in sanityCheck.Select(t => t.Split('|')))
            {
                ...
                if (result != checkTo)
                    throw new Exception(string.Format(
                        "Sanity check on pluralizer failed. Expected \"{0}\" but got \"{1}\".", checkTo, result));
            }
#endif
        }
        ...
        public static int Length(this string input)
        {
            ...
            for (var i = 0; i < input.Length; i++)
            {
                ...
                if (c == '<' && input[i + 1] == 'c') //skip over color tags
                    ...
                    i += 3;
                else if ((c >= 0x3000 && c < 0x4000) || (c >= 0x4E00 && c < 0xA000) ||
                         (c >= 0xE400 && c < 0xE500)) //report double the length for Japanese
                    ret += 2;
        ...
        public static string Viewpoint(this string message, Character top, Character bottom = null)
        {
            ...
            var wordStructFilter = new Func<Token, Character, bool>((filter, who) =>
            {
                ...
                env.pussyWetness = who.HasToken("vagina") && who.GetToken("vagina").HasToken("wetness")
                    ? who.GetToken("vagina").GetToken("wetness").Value
                    : 0;
            ...
            var regex = new Regex(@"{(?:{)? (?<first>\w*)   (?: \| (?<second>\w*) )? }(?:})?",
                RegexOptions.IgnorePatternWhitespace | RegexOptions.Multiline);
            message = regex.Replace(message,
                (match => top == player
                    ? (match.Groups["second"].Success ? match.Groups["second"].Value : string.Empty)
...
\]", RegexOptions.IgnorePatternWhitespace | RegexOptions.Multiline);
            ...
            while (regex.IsMatch(message))
            {
                message = regex.Replace(message, (match =>
                {
                    ...
                    if (targetGroup.StartsWith('?'))
                    {
                        ...
                        var pRes = pTok.Tokens.Where(x =>
                            !x.HasToken("filter") || wordStructFilter(x.GetToken("filter"), target)).ToList();
            ...
            message = Regex.Replace(message, @"\[\!(?<keybinding>.+?)\]",
                (match => Toolkit.TranslateKey(match.Groups["keybinding"].Value)));
            ...
            if (speechFilter == null)
            {
                ...
                if (speechFilter == null) //Still?
                    speechFilter =
                        new SpeechFilter(x =>
                            x); //Then just assign a dummy so we don't do this all over and over again.
