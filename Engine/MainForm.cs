...
namespace Noxico.Engine
{
    public class MainForm : Form, IGameHost
    {
        ...
        {
            get { return palette.Length > 2; }
        }

        public bool Is437
        {
            get { return fourThirtySeven; }
        }
        ...
        public MainForm()
        {
...
#endif
            {
                ...
                {
                    SystemMessageBox.Show(this, "Could not find game data. Please redownload the game.",
                ...
                for (var i = 1; i < 9; i++)
                {
                    ...
                    cursorPens[2, i] = cursorPens[2, 16 - i] =
                        new Pen(Color.FromArgb((i * 32) - 1, (i * 32) - 1, (i * 32) - 1));
                }
        ...
        public void RestartGraphics(bool full)
        {
            ...
            if (!Mix.FileExists("fonts\\" + pngFont + ".png"))
            {
                ...
                if (!Mix.FileExists("fonts\\" + pngFont + ".png"))
                {
                    SystemMessageBox.Show(this, "Could not find font bitmaps. Please redownload the game.",
                        Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Close();
                    return;
                }
            ...
            if (youtube)
            {
                ...
                if (ClientSize.Width == prime.Width && ClientSize.Height == prime.Height)
                {
                    ...
                    FormBorderStyle = IniFile.GetValue("misc", "border", true)
                        ? System.Windows.Forms.FormBorderStyle.FixedSingle
                        : System.Windows.Forms.FormBorderStyle.None;

                youtubeRect = new System.Drawing.Rectangle((ClientSize.Width / 2) - (eW / 2),
                    (ClientSize.Height / 2) - (eH / 2), eW, eH);
            }
            else
                FormBorderStyle = IniFile.GetValue("misc", "border", true)
                    ? System.Windows.Forms.FormBorderStyle.FixedSingle
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Cursor.X >= 0 && Cursor.X < Program.Cols && Cursor.Y >= 0 && Cursor.Y < Program.Rows)
            {
                ...
                e.Graphics.DrawRectangle(cursorPens[(int) NoxicoGame.Mode, pen], offX + (Cursor.X * cellWidth) - 1,
                    offY + (Cursor.Y * cellHeight) - 1, cSize + 1, cellHeight + 1);
            }
        }
        ...
        public void SetCell(int row, int col, int character, Color foregroundColor, Color backgroundColor,
            ...
            bool darken = false)
        {
            ...
            if (fontVariants == null)
            {
                ...
                foreach (var t in tml)
                    fontVariants.Add(t.Text,
                        Tuple.Create((int) t.GetToken("offset").Value, t.HasToken("hasLowercase")));
            }
                ...
                if (c == '<')
                {
                    ...
                    if (gtPos != -1)
                    {
                        ...
                        if (tag[0] == 'c')
                        {
                            ...
                            foregroundColor =
                                (!string.IsNullOrEmpty(match.Groups["fore"].Value) &&
                                 match.Groups["fore"].Value.Length > 1)
                                    ? Color.FromName(match.Groups["fore"].Value)
                                    : fg;
                            backgroundColor = !string.IsNullOrEmpty(match.Groups["back"].Value)
                                ? Color.FromName(match.Groups["back"].Value)
                                : bg;
                            continue;
                        }
                        else if (tag[0] == 'f')
        ...
        public void Draw()
        {
            var lockData = backBuffer.LockBits(new System.Drawing.Rectangle(0, 0, backBuffer.Width, backBuffer.Height),
            ...
            if (e.KeyCode == (Keys) NoxicoGame.KeyBindings[KeyBinding.Screenshot])
            {
                if (e.Modifiers == Keys.Shift)
                {
                    ...
                    using (var dumpFile = new StreamWriter("lol.html"))
                    {
                        ...
                        dumpFile.WriteLine(
                            "<table style=\"font-family: Unifont, monospace\" cellspacing=0 cellpadding=0>");
                        for (int row = 0; row < Program.Rows; row++)
                        {
                            dumpFile.Write("<tr>");
                            for (int col = 0; col < Program.Cols; col++)
                            {
                                var ch = string.Format("&#x{0:X};",
                                    (int) NoxicoGame.IngameToUnicode[image[col, row].Character]);
                                if (ch == "&#x20;")
                                    ch = "&nbsp;";
                                dumpFile.Write("<td style=\"background:{1};color:{2}\">{0}</td>", ch,
                                    image[col, row].Background.ToHex(), image[col, row].Foreground.ToHex());
                            }
                ...
                if (shotDir.StartsWith('$'))
                    shotDir = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) +
                              shotDir.Substring(1);
        ...
        private void MainForm_MouseUp(object x, MouseEventArgs y)
        {
            ...
            var outside =
                (lptx < 0 || lpty < 0 || lptx > Noxico.CurrentBoard.Width || lpty > Noxico.CurrentBoard.Height);
                ...
                if (tx < Program.Cols && ty < Program.Rows)
                {
                    ...
                    else if (y.Button == MouseButtons.Right)
                    {
                        ...
                        NoxicoGame.Cursor.Update();
                             ...
                             (y.Button == System.Windows.Forms.MouseButtons.Left && outside))
                    {
                        if (lpty < 8)
                        {
