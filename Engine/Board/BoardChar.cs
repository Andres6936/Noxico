...
namespace Noxico
{
    public class BoardChar : Entity
    {
        ...
        {
            get { return (Posture) Character.GetToken("posture").Value; }
            set { Character.GetToken("posture").Value = (float) value; }
        }
        ...
        public virtual void AdjustView()
        {
            ...
            Token forcedGlyph = Character.Path("glyph") ??
                                (Character.HasToken("beast") ? Character.Path("bestiary") : null) ?? null;
            if (forcedGlyph != null)
            {
        ...
        public override object CanMove(Direction targetDirection, SolidityCheck check)
        {
            ...
            if (canMove != null && canMove is bool && !(bool) canMove)
                return canMove;
            if (!Character.HasToken("hostile") && !ScriptPathing &&
                (Character.HasToken("sectorlock") || Character.HasToken("sectoravoid")))
            {
        ...
        public override void Draw()
        {
            ...
            if (localX >= Program.Cols || localY >= Program.Rows || localX < 0 || localY < 0)
                return;
            var b = ((MainForm) NoxicoGame.HostForm).IsMultiColor
                ? TileDefinition.Find(this.ParentBoard.Tilemap[this.XPosition, this.YPosition].Index, true).Background
            ...
            if (ParentBoard.IsLit(this.YPosition, this.XPosition))
            {
                ...
                NoxicoGame.HostForm.SetCell(localY, localX, GlowGlyph, Color.FromName(Character.Path("eyes").Text),
                    ParentBoard.Tilemap[XPosition, YPosition].Definition.Background.Night());
        }
        ...
        */
        ...
        public void CheckForTimedItems()
        {
            foreach (var carriedItem in this.Character.GetToken("items").Tokens)
            {
                ...
                if (timer.Value <= 0)
                {
                    ...
                    {
                        Program.WriteLine("Warning: {0} has a timer, but no OnTimer script! Timer token removed.",
                            carriedItem.Name);
                        carriedItem.RemoveToken("timer");
                        continue;
                    }

                    knownItem.RunScript(carriedItem, knownItem.OnTimer, this.Character, this,
                        (m => NoxicoGame.AddMessage(m)));
                }
            }
        }
        ...
        private void NewMove()
        {
            ...
            {
                preferredTarget = ParentBoard.Entities.OfType<BoardChar>()
                    .FirstOrDefault(x => x.ID == Character.GetToken("huntingtarget").Text);
            ...
            if (target is BoardChar && action == TeamBehaviorAction.Attack)
            {
                ...
                if (weapon != null && !weapon.HasToken("weapon"))
                    weapon = null;
                var range = (weapon == null || weapon.Path("weapon/range") == null)
                    ? 1
                    : (int) weapon.Path("weapon/range").Value;
                ...
                if (distance <= range && CanSee(bcTarget))
                {
                    ...
                    if (IniFile.GetValue("misc", "allowrape", false) && distance == 1 &&
                        bcTarget.Character.HasToken("helpless") && Character.GetStat("excitement") > 30 &&
                        Character.Likes(bcTarget.Character))
                    {
            ...
            if (!CanSee(target) && Character.HasToken("targetlastpos"))
            {
                if (ScriptPathTarget == null)
                {
                    ...
                    ScriptPathTarget.Hotspots.Add(new Point((int) lastPos.GetToken("x").Value,
                        (int) lastPos.GetToken("y").Value));
        ...
        private void ActuallyMove()
        {
            ...
            if (ParentBoard == player.ParentBoard && hostile != null)
            {
                ...
                if (ally)
                    target = ParentBoard.Entities.OfType<BoardChar>().FirstOrDefault(x =>
                        !(x is Player) && x != this && x.Character.HasToken("hostile"));
                        ...
                        if (!ally)
                        {
                            if (Character.HasToken("copier"))
                            {
                                ...
                                if (copier.Value == 0 && !copier.HasToken("timeout"))
                                {
                                    ...
                                    AdjustView();
                                    NoxicoGame.AddMessage(i18n
                                        .Format(Character.HasToken("fullCopy") ? "x_becomes_y" : "x_imitates_y")
                                        .Viewpoint(Character, target.Character));
                                    Energy -= 2000;
                                    return;
                                }
                            }
                            ...
                            if (Character.GetStat("excitement") < 30)
                            {
                                ...
                                var called = 0;
                                foreach (var other in ParentBoard.Entities.OfType<BoardChar>().Where(x =>
                                             !(x is Player) && x != this && DistanceFrom(x) < 10 &&
                                             x.Character.HasToken("hostile")))
                                {
                                    called++;
                                    other.CallTo(player);
                                }
                                ...
                                if (called > 0)
                                {
                                    if (!Character.HasToken("beast"))
                                        NoxicoGame.AddMessage(
                                            i18n.Format("call_out", Character.GetKnownName(true, true, true, true))
                                                .SmartQuote().Viewpoint(this.Character, target.Character),
                                            GetEffectiveColor());
                                    else
                                        NoxicoGame.AddMessage(i18n.Format("call_out_animal").Viewpoint(this.Character),
                                            GetEffectiveColor());
                                    Program.WriteLine("{0} called {1} others to player's location.",
                                        this.Character.Name, called);
                                    Energy -= 2000;
                                }
                            }
                        }
        ...
        private void Hunt()
        {
            ...
            if (Character.HasToken("huntingtarget"))
                target = ParentBoard.Entities.OfType<BoardChar>()
                    .First(x => x.ID == Character.GetToken("huntingtarget").Text);
            ...
            {
                var newTarget = ParentBoard.Entities.OfType<DroppedItem>().FirstOrDefault(x =>
                    x.Token.HasToken("owner") && x.Token.GetToken("owner").Text == Character.ID);
                if (newTarget != null)
                    target = newTarget;
            }
            ...
            if (!CanSee(target) && Character.HasToken("targetlastpos"))
            {
                ...
                {
                    Program.WriteLine("{0} couldn't find target at LKP {1}, wandering...", this.ID,
                        ScriptPathTarget.Hotspots[0].ToString());
        ...
        public virtual bool MeleeAttack(BoardChar target)
        {
            ...
            var baseDamage =
                GetPotentialDamage(target, out skill, out verb, out cause, out weaponData, out carriedWeapon);

            var level = (this.Character.Path("skills/" + skill) == null)
                ? 0
                : (int) this.Character.Path("skills/" + skill).Value;
            var damage = 0.0f;
            ...
            if (dodged)
            {
                NoxicoGame.AddMessage(i18n.Format("x_dodges_ys_attack").Viewpoint(this.Character, target.Character),
                    target.GetEffectiveColor());
                return false;
            }
            ...
            {
                NoxicoGame.AddMessage(
                    i18n.Format("x_verbs_y_for_z", verb, (int) damage).Viewpoint(this.Character, target.Character),
                    target.GetEffectiveColor());
        ...
        public float GetPotentialDamage(BoardChar target, out string skill, out string verb, out string cause,
            out Token weaponData, out Token carriedWeapon)
        {
            weaponData = null;
            carriedWeapon = null;
            ...
            else
            {
                ...
                if (carriedWeapon.HasToken("bonus"))
                    baseDamage =
                        (float) Math.Ceiling(baseDamage * ((carriedWeapon.GetToken("bonus").Value + 1) * 0.75f));
        ...
        public bool ConsiderAttack(BoardChar target)
        {
            ...
            var theirDamage =
                target.GetPotentialDamage(this, out skill, out verb, out cause, out weaponData, out carriedWeapon);
        ...
        public virtual bool Hurt(float damage, string cause, object aggressor, bool finishable = false,
            bool leaveCorpse = true)
        {
            ...
            if (health - damage <= 0)
            {
                if (finishable && !Character.HasToken("beast"))
                {
                    if (!Character.HasToken("helpless"))
                    {
                        NoxicoGame.AddMessage(i18n.Format("x_is_helpless").Viewpoint(Character),
        ...
        {
            if (Character.HasToken("copier") && Character.GetToken("copier").Value > 0 &&
                Character.HasToken("fullCopy"))
            {
                //Revert changelings to their true form first.
                Character.Copy(null);
            }

            var name = i18n.Format("corpse_name",
                (Character.IsProperNamed
                    ? Character.Name.ToString(true)
                    : Character.GetKnownName(true, false, false, true)));
            ...
            if (aggressor != null)
            {
                if (aggressor is BoardChar)
                    obituary = i18n.Format("corpse_xed_by_y", cause,
                        ((BoardChar) aggressor).Character.IsProperNamed
                            ? ((BoardChar) aggressor).Character.Name.ToString(true)
                            : ((BoardChar) aggressor).Character.GetKnownName(true, true, false));
                else
                    obituary = i18n.Format("corpse_xed_by_y", cause, i18n.GetString((string) aggressor));
            }

            corpse.Token.AddToken("description",
                i18n.Format("corpse_description",
                    Character.IsProperNamed ? Character.Name.ToString(true) : Character.GetKnownName(true, true, false),
                    obituary));
            if (!(this is Player))
            {
        ...
        public void ReassignStuff(Name oldName)
        {
            ...
            foreach (var thing in this.ParentBoard.Entities.OfType<Clutter>().Where(x => x.ID.Contains(oldID)))
            {
                ...
                thing.Description =
                    thing.Description.Replace(oldName.ToString(true), this.Character.Name.ToString(true));
        ...
        public void AimShot(Entity target)
        {
            ...
            if (new[] {"throwing", "small_firearm", "large_firearm", "huge_firearm"}.Contains(skill.Text))
            {
                ...
                {
                    var carriedGun = this.Character.GetToken("items").Tokens
                        .Find(ci => ci.Name == weapon.ID && ci.HasToken("equipped"));
            ...
            if (aimSuccess)
            {
                ...
                if (target is BoardChar)
                {
                    ...
                    foreach (var targetArmor in hit.Character.GetToken("items").Tokens
                                 .Where(t => t.HasToken("equipped")))
                    {
                        var targetArmorItem = NoxicoGame.KnownItems.FirstOrDefault(i => i.Name == targetArmor.Name);
                        if (targetArmorItem == null)
                            continue;
                    ...
                    NoxicoGame.AddMessage(
                        i18n.Format("x_verbs_y_for_z", "hit", damage).Viewpoint(this.Character, hit.Character),
                        this.GetEffectiveColor());
        ...
        {
            return this.ParentBoard.Entities.OfType<BoardChar>().Where(e => e != this && e.DistanceFrom(this) < range)
                .ToList();
        }
    }
}