...
namespace Noxico
{
    ...
    public partial class Board : TokenCarrier
    {
        ...
        public int Width
        {
            get { return (int) GetToken("width").Value; }
            set { GetToken("width").Value = value; }
        }

        public int Height
        {
            get { return (int) GetToken("height").Value; }
            set { GetToken("height").Value = value; }
        }
        ...
        public string Name
        {
            get { return GetToken("name").Text; }
            set { GetToken("name").Text = value; }
        }

        public string ID
        {
            get { return GetToken("id").Text; }
            set { GetToken("id").Text = value; }
        }

        public string Music
        {
            get { return GetToken("music").Text; }
            set { GetToken("music").Text = value; }
        }

        public BoardType BoardType
        {
            get { return (BoardType) GetToken("type").Value; }
            set { GetToken("type").Value = (float) value; }
        }

        public int ToNorth
        {
            get { return (int) GetToken("north").Value; }
            set { GetToken("north").Value = value; }
        }

        public int ToSouth
        {
            get { return (int) GetToken("south").Value; }
            set { GetToken("south").Value = value; }
        }

        public int ToEast
        {
            get { return (int) GetToken("east").Value; }
            set { GetToken("east").Value = value; }
        }

        public int ToWest
        {
            get { return (int) GetToken("west").Value; }
            set { GetToken("west").Value = value; }
        }

        public bool AllowTravel
        {
            get { return !HasToken("noTravel"); }
            set
            {
                RemoveToken("noTravel");
                if (!value) AddToken("noTravel");
            }
        }
        ...
        public int Stock
        {
            ...
            get { return Path("encounters/stock") != null ? (int) Path("encounters/stock").Value : 0; }
            set
            {
        ...
        public Realms Realm
        {
            get { return (Realms) GetToken("realm").Value; }
            set { GetToken("realm").Value = (float) value; }
        }
        ...
        public void SaveToFile(int index)
        {
            ...
            using (var stream = new BinaryWriter(File.Open(System.IO.Path.Combine(realm, "Board" + index + ".brd"),
                       FileMode.Create)))
            {
        ...
        public void Update(bool active = false, bool surrounding = false)
        {
            ...
            if (EntitiesToRemove.Count > 0)
            {
                EntitiesToRemove.ForEach(x =>
                {
                    if (x is BoardChar)
                    {
                        ((BoardChar) x).Character.ResetEquipmentCarries();
                    }
                });
                EntitiesToRemove.ForEach(x =>
                {
                    Entities.Remove(x);
                    this.DirtySpots.Add(new Point(x.XPosition, x.YPosition));
                });
        ...
        public void Draw(bool force = false)
        {
            if (Height < Program.Rows || Width < Program.Cols)
            {
                ...
                NoxicoGame.HostForm.SetCell(-NoxicoGame.CameraY - 1, -NoxicoGame.CameraX - 1, (char) 0xC9,
                    Color.DarkGray, Color.Black);
                NoxicoGame.HostForm.SetCell(-NoxicoGame.CameraY - 1, -NoxicoGame.CameraX + Width, (char) 0xBB,
                    Color.DarkGray, Color.Black);
                NoxicoGame.HostForm.SetCell(-NoxicoGame.CameraY + Height, -NoxicoGame.CameraX - 1, (char) 0xC8,
                    Color.DarkGray, Color.Black);
                NoxicoGame.HostForm.SetCell(-NoxicoGame.CameraY + Height, -NoxicoGame.CameraX + Width, (char) 0xBC,
                    Color.DarkGray, Color.Black);
                for (var c = -NoxicoGame.CameraX; c < -NoxicoGame.CameraX + Width; c++)
                {
                    ...
                    NoxicoGame.HostForm.SetCell(-NoxicoGame.CameraY + Height, c, (char) 0xCD, Color.DarkGray,
                        Color.Black);
                }
                ...
                for (var r = -NoxicoGame.CameraY; r < -NoxicoGame.CameraY + Height; r++)
                {
                    ...
                    NoxicoGame.HostForm.SetCell(r, -NoxicoGame.CameraX + Width, (char) 0xBA, Color.DarkGray,
                        Color.Black);
                }
            }
            ...
            var waterColors = new[]
            {
                Color.Black, Color.Navy, Color.FromCSS("B22222"), Color.Black, Color.Red, Color.White, Color.Black,
                Color.Black
            };
            while (this.DirtySpots.Count > 0)
            {
                ...
                {
                    fore = Color.FromArgb(Random.Next(20, 25) * 10, Random.Next(5, 25) * 10,
                        0); //flameColors[Randomizer.Next(flameColors.Length)];
                    back = Color.FromArgb(Random.Next(20, 25) * 10, Random.Next(5, 25) * 10,
                        0); //flameColors[Randomizer.Next(flameColors.Length)];
                }
        ...
        {
            if ( /* (source != null && source is BoardChar && ((BoardChar)source).Character.Path("eyes/glow") != null) || */
                (!HasToken("dark") && !Toolkit.IsNight()))
            {
                for (int row = 0; row < Height; row++)
                for (int col = 0; col < Width; col++)
            ...
            if (source != null)
                SilverlightShadowCasting.ShadowCaster.ComputeFieldOfViewWithShadowCasting(source.XPosition,
                    source.YPosition, radius, f, a);
            if (torches)
            {
                doingTorches = true;
                foreach (var light in Entities.OfType<Clutter>().Where(x => x.ID.Contains("Torch")))
                {
                    SilverlightShadowCasting.ShadowCaster.ComputeFieldOfViewWithShadowCasting(light.XPosition,
                        light.YPosition, 5, f, a);
                }
            }
        ...
        public static Board CreateBasicOverworldBoard(int biomeID, string id, string name, string music, int width,
            int height)
        {
        ...
        public void DumpToHtml(string suffix)
        {
            ...
            if (Entities.OfType<BoardChar>().Count() > 0)
            {
                ...
                {
                    file.WriteLine("<h4 id=\"{1}_{2}x{3}\">{0}</h4>",
                        bc.Character.IsProperNamed ? bc.Character.Name.ToString(true) : bc.Character.Title, bc.ID,
                        bc.XPosition, bc.YPosition);
                    file.WriteLine("<pre>");
            ...
            if (Entities.OfType<Container>().Count() > 0)
            {
                ...
                {
                    file.WriteLine("<h4 id=\"{3}_{1}x{2}\">{0} at {1}x{2}</h4>", c.Name, c.XPosition, c.YPosition,
                        c.ID);
                    file.WriteLine("<pre>");
        ...
        public BoardChar PickBoardChar(Gender gender)
        {
            ...
            if (gender != Gender.Invisible)
                isOkay = (x) => { return (x.Character.Gender == gender); };
        ...
        public void CreateHtmlScreenshot(StreamWriter stream, bool linked)
        {
            ...
            stream.WriteLine(
                "<table style=\"font-family: Unifont, monospace; cursor: default;\" cellspacing=0 cellpadding=0>");
            for (int row = 0; row < Height; row++)
            {
                stream.WriteLine("\t<tr>");
                for (int col = 0; col < Width; col++)
                {
                    ...
                    if (tile.Fluid != Fluids.Dry)
                    {
                        chr = string.Format("&#x{0:X};",
                            (int) NoxicoGame.IngameToUnicode[waterGlyphs[(int) tile.Fluid]]);
                    ...
                    if (ent != null)
                    {
                        back = string.Format("rgb({0},{1},{2})", ent.BackgroundColor.R, ent.BackgroundColor.G,
                            ent.BackgroundColor.B);
                        fore = string.Format("rgb({0},{1},{2})", ent.ForegroundColor.R, ent.ForegroundColor.G,
                            ent.ForegroundColor.B);
                        ...
                        if (ent is BoardChar)
                        {
                            ...
                            if (linked)
                                link = string.Format("<a href=\"#{0}_{1}x{2}\" style=\"color: {3};\">", ent.ID,
                                    ent.XPosition, ent.YPosition, fore);
                        }
                    ...
                    stream.WriteLine("\t\t<td style=\"background: {0}; color: {1};\"{3}>{4}{2}{5}</td>", back, fore,
                        chr, tag, link, link.IsBlank(string.Empty, "</a>"));
        ...
        public void ResolveVariableWalls()
        {
            ...
            for (int row = 0; row < Height; row++)
            {
                for (int col = 0; col < Width; col++)
                {
                    ...
                    {
                        var baseName = def.Name.Replace("Top", string.Empty).Replace("Bottom", string.Empty)
                            .Replace("Left", string.Empty).Replace("Right", string.Empty)
                            .Replace("Joiner", string.Empty);
                        newTile.Clear();
                        newTile.Append(baseName);
                        if (row > 0 && (Tilemap[col, row - 1].Definition.Name.StartsWith(baseName) ||
                                        Tilemap[col, row - 1].Definition.Name.StartsWith("doorway")))
                            newTile.Append("Top");
                        if (row < Height - 1 && (Tilemap[col, row + 1].Definition.Name.StartsWith(baseName) ||
                                                 Tilemap[col, row + 1].Definition.Name.StartsWith("doorway")))
                            newTile.Append("Bottom");
                        if (col > 0 && (Tilemap[col - 1, row].Definition.Name.StartsWith(baseName) ||
                                        Tilemap[col - 1, row].Definition.Name.StartsWith("doorway")))
                            newTile.Append("Left");
                        if (col < Width - 1 && (Tilemap[col + 1, row].Definition.Name.StartsWith(baseName) ||
                                                Tilemap[col + 1, row].Definition.Name.StartsWith("doorway")))
