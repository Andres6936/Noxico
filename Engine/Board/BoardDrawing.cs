...
namespace Noxico
{
    public partial class Board : TokenCarrier
    {
            ...
            for (var y = 0; y < height; y++)
            {
                for (var x = 0; x < width; x++)
                {
                        ...
                        foreach (var property in properties.Tokens)
                        {
                            switch (property.Name)
                            {
                                case "id":
                                    ...
                                    break;
                                case "name":
                                    nc.Name = property.Text;
                                    break;
                                case "desc":
                                    nc.Description = property.Text;
                                    break;
                                case "glyph":
                                    nc.Glyph = (int) property.Value;
                                    break;
                                case "fg":
                    ...
                    if (!tile.HasToken("fluid"))
                        ...
                        this.Tilemap[x, y].Fluid =
                            (Fluids) Enum.Parse(typeof(Fluids), tile.GetToken("fluid").Text, true);
                }
            }
        ...
        {
            AddWater(new List<Rectangle>()
                {new Rectangle() {Left = 0, Top = 0, Right = this.Width - 1, Bottom = this.Height - 1}});
        }
        ...
        public void GenerateTown(bool rename, bool vendors, List<string> vendorTypes)
        {
            ...
            if (vendors)
            {
                ...
                var citizens = this.Entities.OfType<BoardChar>().Where(e => e.Character.Path("role/vendor") == null)
                    .ToList();
                foreach (var vendorType in vendorTypes)
                {
                    ...
                    Program.WriteLine("*** {0} of {2} is now a {1} ***", newVendor.Name.ToString(true), vendorType,
                        this.Name);
