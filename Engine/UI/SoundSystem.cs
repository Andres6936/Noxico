    ...
    public class SoundSystem
    {
        ...
        public void PlayMusic(string name, bool fade = true)
        {
            ...
            else if (musicPlaying != name)
            {
                if (Mix.FileExists(name))
                {
                    ...
                    CheckError(system.CreateSound(data, FMOD.SoundMode.LoopNormal | FMOD.SoundMode.OpenMemory,
                        ref fCSex, ref music));
        ...
        public void Update()
        {
            ...
            if (type == FMOD.SoundType.Mp3 | type == FMOD.SoundType.OggVorbis)
            {
                ...
                debug = string.Format("{0}ch {1}, {2} / {3}. Volume: {4:P0}", musicchans, type.ToString(),
                    new TimeSpan(ms * 10000), new TimeSpan(len * 10000), volume);
            }
            else
            {
                var row = 0u;
                ...
                debug = string.Format("{0}ch {1}, row {2}/{3}, pattern {4}/{5}. Volume: {6:P0}", musicchans,
                    type.ToString(), row, rows, pattern, patterns, volume);
            }

            NoxicoGame.HostForm.Write(debug.PadRight(Program.Cols), System.Drawing.Color.Gray,
        ...
        private class Sound
        {
            ...
            public Sound(string file, FMOD.System system)
            {
                ...
                SoundSystem.CheckError(system.CreateSound(data, FMOD.SoundMode.Default | FMOD.SoundMode.OpenMemory,
                    ref fCSex, ref ns));
                InnerSound = ns;
            }
