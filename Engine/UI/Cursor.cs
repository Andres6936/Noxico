...
namespace Noxico
{
    public class Cursor : Entity
    {
        ...
        public void Point()
        {
            ...
            foreach (var entity in this.ParentBoard.Entities)
            {
                if (entity.XPosition == XPosition && entity.YPosition == YPosition)
                {
                    if (!this.ParentBoard.IsLit(YPosition, XPosition) && NoxicoGame.Me.Player.CanSee(entity))
                    {
                        if (NoxicoGame.Me.Player.Character.Path("eyes/glow") == null)
                        {
                            ...
                            if (entity is BoardChar && ((BoardChar) entity).Character.Path("eyes/glow") != null)
                            {
                                ...
                                NoxicoGame.LookAt = "<c" + ((BoardChar) entity).Character.Path("eyes").Text + ">" +
                                                    i18n.GetString("eyesinthedark");
                            }
                    ...
                    {
                        NoxicoGame.LookAt = string.Format("<c{0}>{1}<c> ({2}/{3})",
                            ((BoardChar) entity).Character.Path("skin/color").Text,
                            ((BoardChar) PointingAt).Character.GetKnownName(true, true),
                            ((BoardChar) PointingAt).Character.Health,
                            ((BoardChar) PointingAt).Character.MaximumHealth);
                        //return;
                    }
                    ...
                    {
                        var desc = (entity is Container
                            ? ((Container) PointingAt).Description
                            : ((Clutter) PointingAt).Description);
        ...
        public override void Update()
        {
            ...
            if (NoxicoGame.IsKeyDown(KeyBinding.Interact) || NoxicoGame.IsKeyDown(KeyBinding.Back) ||
                Vista.Triggers == XInputButtons.B)
            {
                NoxicoGame.Mode = UserMode.Walkabout;
            ...
            if (NoxicoGame.IsKeyDown(KeyBinding.Accept) || Vista.Triggers == XInputButtons.A)
            {
                            ...
                            switch (ActionList.Answer as string)
                            {
                                ...
                                case "spawn":
                                    ...
                                    foreach (var bp in uniques)
                                        spawnOptions['!' + bp.Text] = bp.Text;
                                    ActionList.Show("Debug?", this.XPosition - NoxicoGame.CameraX,
                                        this.YPosition - NoxicoGame.CameraY, spawnOptions,
                                        () =>
                                        {
                                            if (ActionList.Answer is int && (int) ActionList.Answer == -1)
                                            {
                                            ...
                                            if (ActionList.Answer is string &&
                                                ((string) ActionList.Answer).StartsWith('!'))
                                                newChar = Character.GetUnique(((string) ActionList.Answer)
                                                    .Substring(1));
                                            else
                                                newChar = Character.Generate((string) ActionList.Answer,
                                                    Gender.RollDice);
                ...
                if (PointingAt != null)
                {
                    ...
                    if (PointingAt is Player)
                    {
                        ...
                        {
                            if (player.Character.Path("copier/backup") != null ||
                                player.Character.Path("copier/full") == null)
                    ...
                    else if (PointingAt is BoardChar)
                    {
                        ...
                        if (canSee && player.Character.GetStat("excitement") >= 30 && distance <= 1)
                        {
                            ...
                            if (mayFuck)
                                options["fuck"] = i18n.Format(willRape ? "action_rapehim" : "action_fuckhim",
                                    boardChar.Character.HimHerIt(true));
                        }

                        //TODO: This needs to be updated to work with Powers.
                        if (canSee && !boardChar.Character.HasToken("beast") && player.Character.HasToken("copier") &&
                            player.Character.Path("copier/timeout") == null)
                        {
                    ...
                    ActionList.Show(description, PointingAt.XPosition - NoxicoGame.CameraX,
                        PointingAt.YPosition - NoxicoGame.CameraY, options,
                        () =>
                        {
                            Hide();
                            if (ActionList.Answer is int && (int) ActionList.Answer == -1)
                            ...
                            switch (ActionList.Answer as string)
                            {
                                case "look":
                                    if (PointingAt is DroppedItem)
                                    {
                                        ...
                                        var item = drop.Item;
                                        var token = drop.Token;
                                        var text = (item.HasToken("description") && !token.HasToken("unidentified")
                                            ? item.GetToken("description").Text
                                            : i18n.Format("thisis_x", item.ToString(token))).Trim();
                                    ...
                                    else if (PointingAt is Clutter && !((Clutter) PointingAt).Description.IsBlank())
                                    {
                                        MessageBox.Notice(((Clutter) PointingAt).Description.Trim(), true,
                                            ((Clutter) PointingAt).Name ?? "something");
                                    }
                                    else if (PointingAt is Container)
                                    {
                                        MessageBox.Notice(((Container) PointingAt).Description.Trim(), true,
                                            ((Container) PointingAt).Name ?? "container");
                                    }
                                    else if (PointingAt is BoardChar)
                                    {
                                        if (((BoardChar) PointingAt).Character.HasToken("beast"))
                                            MessageBox.Notice(((BoardChar) PointingAt).Character.LookAt(PointingAt),
                                                true, ((BoardChar) PointingAt).Character.GetKnownName(true));
                                        else
                                ...
                                case "talk":
                                    ...
                                    else if (PointingAt is BoardChar)
                                    {
                                        ...
                                        if (boardChar.Character.HasToken("hostile") &&
                                            !boardChar.Character.HasToken("helpless"))
                                            MessageBox.Notice(
                                                i18n.Format("nothingtosay",
                                                    boardChar.Character.GetKnownName(false, false, true, true)), true);
                                        else
                                ...
                                case "copy":
                                    ...
                                    NoxicoGame.AddMessage(i18n
                                        .Format(player.Character.HasToken("fullCopy") ? "x_becomes_y" : "x_imitates_y")
                                        .Viewpoint(player.Character, ((BoardChar) PointingAt).Character));
                                    player.Energy -= 2000;
                                    break;
                                ...
                                case "revert":
                                    ...
                                    player.AdjustView();
                                    NoxicoGame.AddMessage(i18n.GetString((player.Character.HasToken("fullCopy"))
                                        ? "youmelt"
                                        : "yourevert"));
                                    player.Energy -= 1000;
                                    break;
                                ...
                                case "take":
                                    if (PointingAt is DroppedItem)
                                    {
                                        ...
                                        player.Energy -= 1000;
                                        NoxicoGame.AddMessage(i18n.Format("youpickup_x", item.ToString(token, true)),
                                            drop.ForegroundColor);
                                ...
                                case "edit":
                                    ...
                                    NoxicoGame.HostForm.Write("TOKEN EDIT ENGAGED. Waiting for editor process to exit.",
                                        Color.Black, Color.White, 0, 0);
                                    NoxicoGame.HostForm.Draw();
                                    ((MainForm) NoxicoGame.HostForm).timer.Enabled = false;
                                    var dump =
                                        "-- WARNING! Many things may cause strange behavior or crashes. WATCH YOUR FUCKING STEP.\r\n" +
                                ...
                                default:
                                    MessageBox.Notice(
                                        "Unknown action handler \"" + ActionList.Answer.ToString() + "\".", true);
                                    break;
                            }
                        }
                    ); //	true, true);
            ...
            if (NoxicoGame.KeyMap[Keys.D])
            {
                ...
                if (PointingAt != null && PointingAt is BoardChar)
                {
                    ...
                    NoxicoGame.AddMessage(
                        "Info for " + ((BoardChar) PointingAt).Character.GetKnownName(true, true, true) + " dumped.",
                        Color.Red);
                }
            }
#endif
