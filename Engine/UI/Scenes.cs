...
namespace Noxico
{
    class SceneSystem
    {
        ...
        public static void Engage(Character top, Character bottom, string name = "(start)")
        {
            ...
            {
                MessageBox.Notice(
                    string.Format("Could not find a proper opening for scene name \"{0}\". Aborting.", name), true,
                    "Uh-oh.");
                return;
            }
            ...
            if (firstScene.HasToken("random"))
            {
                ...
                foreach (var s in openings.Where(i =>
                             i != firstScene && i.HasToken("random") && i.GetToken("random").Text == randomKey &&
                             SexManager.LimitsOkay(actors, i)))
                    scenes.Add(s);
            }
            ...
            if (bottom == NoxicoGame.Me.Player.Character && !letBottomChoose)
            {
                ...
                else
                {
                    ...
                    MessageBox.List(message, actions,
                        () => { Engage(SceneSystem.top, SceneSystem.bottom, (string) MessageBox.Answer); }, false, true,
                        bottom.GetKnownName(true, true));
                }
            }
            else
            {
                ...
                if (actions.Count == 0)
                    ...
                    MessageBox.List(message, actions,
                        () => { Engage(SceneSystem.top, SceneSystem.bottom, (string) MessageBox.Answer); }, false,
                        !dreaming, bottom.GetKnownName(true, true));
            }
        ...
        private static Dictionary<object, string> ExtractActions(Token scene)
        {
            ...
            {
                foreach (var s in sceneList.Where(x =>
                             x.Name == "scene" && x.GetToken("name").Text == action.Text &&
                             SexManager.LimitsOkay(actors, x)))
                {
                    var key = action.Text;
                    var listAs = s.HasToken("list")
                        ? s.GetToken("list").Text
                        : string.Format("[missing \"list\"!] {0}", key);
