        ...
        public static void Handler()
        {
            ...
            if (Subscreens.Redraw)
            {
                    ...
                    */
                    ...
                    for (var i = 0; i < sigils.Count; i++)
                    {
                        if (sigilText.Length < Program.Cols - 40)
                        {
                            ...
                            sigilText.Append((sigils[i][0] == '\uE300')
                                ? sigils[i].Substring(1)
                                : i18n.GetString("sigil_" + sigils[i]));
                            sigilComma = ", ";
                        }
                        else
                        {
                ...
                if (UIManager.Elements.Count < 2)
                {
                    ...
                    {
                        Width = Program.Cols - 4, Height = height, Index = selection,
                    ...
                    itemList.Change = (s, e) =>
                    {
                        ...
                        if (weaponData != null)
                        {
                            if (weaponData.HasToken("skill"))
                                d += i18n.Format("inventory_weaponskill",
                                    weaponData.GetToken("skill").Text.Replace('_', ' ') + "    ");
                            if (weaponData.HasToken("attacktype"))
                                d += i18n.Format("inventory_weapontype",
                                    i18n.GetString("attacktype_" + weaponData.GetToken("attacktype").Text));
                            d += '\n';
                        }
                    ...
                    UIManager.Elements.Add(
                        new UIButton(' ' + i18n.GetString("inventory_drop") + ' ',
                            (s, e) =>
                            {
