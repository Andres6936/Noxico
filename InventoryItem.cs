...
namespace Noxico
{
    public class InventoryItem : TokenCarrier
    {
        ...
        {
            if (ID == "book" && token != null && token.HasToken("id") &&
                NoxicoGame.BookTitles.ContainsKey(token.GetToken("id").Text))
            ...
            var color = (token != null && token.HasToken("color"))
                ? Color.NameColor(token.GetToken("color").Text)
                : string.Empty;
        ...
        public string GetDescription(Token token)
        {
            ...
            if (this.ID == "book" && token != null && token.HasToken("id") &&
                NoxicoGame.BookTitles.ContainsKey(token.GetToken("id").Text))
                return i18n.Format("book_description", NoxicoGame.BookTitles[token.GetToken("id").Text][0],
                    NoxicoGame.BookTitles[token.GetToken("id").Text][1]);
            var a = string.Empty;
        ...
        public bool CanSeeThrough(Character who)
        {
            ...
            if (CarriedToken.ContainsKey(who.ID) &&
                (CarriedToken[who.ID].HasToken("torn") || CarriedToken[who.ID].HasToken("wet")))
                return true;
        ...
        public bool Equip(Character character, Token item)
        {
            ...
            */
            ...
            if (equip.HasToken("pants") || equip.HasToken("underpants") || equip.HasToken("shoes") ||
                equip.HasToken("socks"))
            ...
            foreach (var t in equip.Tokens)
            {
                if (t.Name == "underpants" && (!TempRemove(character, tempRemove, "pants") ||
                                               !TempRemove(character, tempRemove, "underpants")))
                    return false;
                else if (t.Name == "undershirt" && (!TempRemove(character, tempRemove, "shirt") ||
                                                    !TempRemove(character, tempRemove, "undershirt")))
                    return false;
                else if (t.Name == "shirt" && (!TempRemove(character, tempRemove, "shirt") ||
                                               !TempRemove(character, tempRemove, "jacket")))
                    return false;
                else if (t.Name == "jacket" && (!TempRemove(character, tempRemove, "cloak") ||
                                                !TempRemove(character, tempRemove, "jacket")))
                    return false;
                else if (t.Name == "socks" && (!TempRemove(character, tempRemove, "shoes") ||
                                               !TempRemove(character, tempRemove, "socks")))
                    return false;
            }
        ...
        public bool Unequip(Character character)
        {
            ...
            */
            ...
            if (item != null && item.HasToken("cursed") && item.GetToken("cursed").HasToken("known"))
                throw new ItemException(item.GetToken("cursed").Text
                    .IsBlank(i18n.Format("cannot_remove_sticky", this.ToString(item, true)),
                        item.GetToken("cursed").Text));
        ...
        private bool TempRemove(Character character, Stack<Token> list, string slot)
        {
            foreach (var carriedItem in character.GetToken("items").Tokens)
            {
                ...
                if (equip == null)
                {
                    System.Windows.Forms.MessageBox.Show("Item " + carriedItem.Name + " is marked as equipped, but " +
                                                         find.Name + " is not equippable.");
        ...
        public void Drop(BoardChar boardChar, Token item)
        {
            ...
            if (tile.Fluid != Fluids.Dry || tile.Definition.Cliff)
                NoxicoGame.AddMessage(i18n
                    .Format(
                        tile.Definition.Cliff
                            ? "x_dropped_y_inthedepths"
                            : ("x_dropped_y_inthe_" + tile.Fluid.ToString().ToLowerInvariant()),
                        this.ToString(item, true, false)).Viewpoint(boardChar.Character));
            if (lives == 0)
            {
        ...
        {
            var boardchar = NoxicoGame.Me.CurrentBoard.Entities.OfType<BoardChar>()
                .First(x => x.Character == character);
            ...
            if (this.HasToken("equipable"))
            {
                ...
                else
                {
                    ...
                    {
                        runningDesc += item.GetToken("cursed").Text
                            .IsBlank(
                                i18n.Format("inventory_cursed_" + (this.HasToken("plural") ? "plural" : "singular"),
                                    this.ToString(item, true)), item.GetToken("cursed").Text);
                        showDesc(runningDesc.Viewpoint(boardchar.Character));
                        return;
                    }
        ...
        public object RunScript(Token item, string script, Character character, BoardChar boardchar,
            Action<string> running)
        {
            ...
            env.Consume =
                new Action<string>(
                    x => this.Consume(character, item) /* character.GetToken("items").Tokens.Remove(item) */);
        ...
        public List<string> GetModifiers(Token token)
        {
            ...
            //if (HasToken("equipable"))
            {
                if (HasToken("weapon"))
                {
                    ...
                    info.Add(damage + " dmg");
                    if (new[] {"throwing", "small_firearm", "large_firearm", "huge_firearm"}.Contains(
                            Path("weapon/skill").Text))
