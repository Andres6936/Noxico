    ...
    public class TextScroller
    {
        ...
        public static void Handler()
        {
            ...
            if (Subscreens.FirstDraw)
            {
                ...
                host.Write(help, UIColors.WindowBorder, Color.Transparent, Program.Rows - 3,
                    (Program.Cols / 2) - (help.Length() / 2));
                ...
                for (int i = scroll; i < text.Length && i - scroll < Program.Rows - 5; i++)
                {
                    if (i < 1)
                        continue;
                    host.Write(' ' + text[i].PadEffective(70), UIColors.RegularText, UIColors.WindowBackground, 1 + i,
                        left + 2);
                }
            ...
            {
                NoxicoGame.HostForm.SetCell(2, left + 73, (scroll > 1) ? '\x1E' : '\xBA',
                    (scroll > 1) ? UIColors.RegularText : UIColors.WindowBorder, UIColors.WindowBackground);
                NoxicoGame.HostForm.SetCell(Program.Rows - 4, left + 73, (scroll + 21 < text.Length) ? '\x1F' : '\xBA',
                    (scroll + 21 < text.Length) ? UIColors.RegularText : UIColors.WindowBorder,
            ...
            if ((NoxicoGame.IsKeyDown(KeyBinding.ScrollUp) || Vista.DPad == XInputButtons.Up) &&
                (DateTime.Now - slow).Milliseconds >= 100)
            {
                slow = DateTime.Now;
                scroll--;
                if (scroll < 1)
            ...
            if ((NoxicoGame.IsKeyDown(KeyBinding.ScrollDown) || Vista.DPad == XInputButtons.Down) &&
                (DateTime.Now - slow).Milliseconds >= 100)
            {
                slow = DateTime.Now;
                scroll++;
                ...
                else
                {
                    ...
                    host.Write(new string(' ', 72), UIColors.RegularText, UIColors.WindowBackground, Program.Rows - 4,
                        left + 1);
                    if (i < text.Length)
                        host.Write(text[i], UIColors.RegularText, UIColors.WindowBackground, Program.Rows - 4,
                            left + 2);
                    Subscreens.Redraw = true;
                }
            }
        ...
        public static void LookAt(BoardChar target)
        {
            ...
            Plain(chr.LookAt(pa), chr.GetKnownName(true), false,
                true); //Fix: disabled wrapping to prevent Look At from looking like shit with new wrapper.
        }
        ...
        public static void ReadBook(string bookID)
        {
            ...
            */
            ...
            {
                foreach (var item in NoxicoGame.KnownItems.Where(ki =>
                             ki.HasToken("identify") && ki.GetToken("identify").Text == identification &&
                             !NoxicoGame.Identifications.Contains(ki.ID)))
