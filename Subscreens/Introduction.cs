    ...
    public class Introduction
    {
        ...
        public static void TitleHandler()
        {
            ...
            if (Subscreens.FirstDraw)
            {
                ...
                var background = new Bitmap(Program.Cols, Program.Rows * 2,
                ...
                var pressEnter = "\xC4\xC4\xC4\xC4\xB4 " + i18n.GetString("ts_pressentertobegin") +
                                 " <cGray>\xC3\xC4\xC4\xC4\xC4";
            ...
            if (NoxicoGame.IsKeyDown(KeyBinding.Accept) || Subscreens.Mouse || Vista.Triggers != 0)
            {
                ...
                var options = saves.ToDictionary(new Func<string, object>(s => Path.GetFileName(s)),
                ...
                MessageBox.List(
                    saves.Count == 0
                        ? i18n.GetString("ts_welcometonoxico")
                        : i18n.GetString(saves.Count == 1 ? "ts_thereisasave" : "ts_therearesaves"), options,
                    () =>
                    {
                        ...
                        else if ((string) MessageBox.Answer == "~~")
                        {
                            ...
                            me.CreatePlayerCharacter(env.TestArena.Name, env.TestArena.BioGender,
                                env.TestArena.IdentifyAs, env.TestArena.Preference, env.TestArena.Bodyplan,
                                new Dictionary<string, string>(), env.TestArena.BonusTrait);
        ...
        private static void CollectPlayables()
        {
            ...
            foreach (var bodyPlan in Character.Bodyplans.Where(t => t.Name == "bodyplan"))
            {
                ...
                foreach (var aspect in editables.Split(','))
                {
                    ...
                    colorItems.Add("lbl-" + path,
                        new UILabel(label) {Left = labelL, Top = top, Foreground = Color.Gray});
                    ...
                    if (label.EndsWith("color", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        colorItems.Add(path,
                            new UIColorList()
                            {
                                Items = items, Left = cntrlL, Top = top + 1, Width = width, Foreground = Color.Black,
                                Background = Color.Transparent, Index = 0
                            });
                    }
                    else
                        colorItems.Add(path,
                ...
                playables.Add(new PlayableRace()
                    {ID = id, Name = name, Bestiary = bestiary, ColorItems = colorItems, SexLocks = sexlocks});
            }
        }
        ...
        public static void CharacterCreator()
        {
            if (Subscreens.FirstDraw)
            {
                ...
                backdrop = new Bitmap(Program.Cols, Program.Rows * 2,
                    System.Drawing.Imaging.PixelFormat.Format24bppRgb);
                ...
                string[] sexoptions =
                {
                    i18n.GetString("Male"), i18n.GetString("Female"), i18n.GetString("Herm"), i18n.GetString("Neuter")
                };
                ...
                controls = new Dictionary<string, UIElement>()
                {
                    ...
                    {"headerline", new UILabel(bar) {Left = labelL, Top = header, Foreground = Color.Black}},
                    {
                        ...
                        new UILabel(title)
                        {
                            Left = labelL + 17 - (title.Length() / 2), Top = header, Width = title.Length(),
                            Foreground = Color.Black
                        }
                    },
                    {
                        "back",
                        new UIButton(i18n.GetString("cc_back"), null)
                            {Left = labelL, Top = buttons, Width = 10, Height = 1}
                    },
                    {
                        "next",
                        new UIButton(i18n.GetString("cc_next"), null)
                            {Left = labelL + 23, Top = buttons, Width = 10, Height = 1}
                    },
                    {
                        "wait",
                        new UILabel(i18n.GetString("cc_wait"))
                            {Left = labelL + 23, Top = buttons, Foreground = Color.Silver}
                    },
                    {
                        "play",
                        new UIButton(i18n.GetString("cc_play"), null)
                            {Left = labelL + 23, Top = buttons, Width = 10, Height = 1, Hidden = true}
                    },

                    {
                        "nameLabel",
                        new UILabel(i18n.GetString("cc_name")) {Left = labelL, Top = top, Foreground = Color.Gray}
                    },
                    {
                        "name",
                        new UITextBox(string.Empty)
                        {
                            Left = cntrlL, Top = top + 1, Width = 24, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    {
                        "nameRandom",
                        new UILabel(i18n.GetString("cc_random"))
                            {Left = cntrlL + 2, Top = top + 1, Foreground = Color.Gray}
                    },
                    {
                        "speciesLabel",
                        new UILabel(i18n.GetString("cc_species"))
                    ...
                    },
                    {
                        ...
                        new UISingleList()
                        {
                            Left = cntrlL, Top = top + 4, Width = 26, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    {
                        "tutorial",
                        new UIToggle(i18n.GetString("cc_tutorial"))
                        {
                            Left = labelL, Top = top + 6, Width = 24, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    {
                        "easy",
                        new UIToggle(i18n.GetString("cc_easy"))
                        {
                            Left = labelL, Top = top + 7, Width = 24, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    ...
                    {
                        "sexLabel",
                        new UILabel(i18n.GetString("cc_sex")) {Left = labelL, Top = top, Foreground = Color.Gray}
                    },
                    {
                        "sex",
                        new UIRadioList(sexoptions)
                        {
                            Left = cntrlL, Top = top + 1, Width = 24, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    ...
                    {
                        "gidLabel",
                        new UILabel(i18n.GetString("cc_gid")) {Left = labelL, Top = top + 6, Foreground = Color.Gray}
                    },
                    {
                        "gid",
                        new UISingleList(string.Empty, null, sexoptions.ToList(), 0)
                        {
                            Left = cntrlL, Top = top + 7, Width = 26, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    {
                        "prefLabel",
                        new UILabel(i18n.GetString("cc_pref")) {Left = labelL, Top = top + 9, Foreground = Color.Gray}
                    },
                    {
                        "pref",
                        new UISingleList(string.Empty, null, prefoptions.ToList(), 1)
                        {
                            Left = cntrlL, Top = top + 10, Width = 26, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    ...
                    {
                        "giftLabel",
                        new UILabel(i18n.GetString("cc_gift")) {Left = labelL, Top = top, Foreground = Color.Gray}
                    },
                    {
                        "gift",
                        new UIList(string.Empty, null, traits)
                        {
                            Left = cntrlL, Top = top + 1, Width = 30, Height = 24, Foreground = Color.Black,
                            Background = Color.Transparent
                        }
                    },
                    ...
                    {
                        "topHeader",
                        new UILabel(i18n.GetString("cc_header")) {Left = 1, Top = 0, Foreground = Color.Silver}
                    },
                    {
                        "helpLine",
                        new UILabel(i18n.GetString("cc_footer"))
                ...
                pages = new List<UIElement>[]
                {
                    new List<UIElement>()
                    {
                        controls["backdrop"], controls["headerline"], controls["header"], controls["topHeader"],
                        controls["helpLine"],
                    ...
                    new List<UIElement>()
                    {
                        ...
                        controls["prefLabel"], controls["pref"],
                ...
                controls["back"].Enter = (s, e) =>
                {
                    page--;
                    loadPage(page);
                    UIManager.Draw();
                };
                controls["next"].Enter = (s, e) =>
                {
                    page++;
                    loadPage(page);
                    UIManager.Draw();
                ...
                controls["play"].Enter = (s, e) =>
                {
                    ...
                    NoxicoGame.Me.CreatePlayerCharacter(playerName.Trim(), (Gender) (sex + 1), (Gender) (gid + 1), pref,
                        playables[species].ID, colorMap, bonus);
                    if (tutorial)
