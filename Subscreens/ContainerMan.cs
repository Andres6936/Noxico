    ...
    public class ContainerMan
    {
        ...
        public static void Handler()
        {
            ...
            if (Subscreens.FirstDraw)
            {
                ...
                var emptyMessage = mode == ContainerMode.Vendor
                    ?
                    i18n.Format("inventory_x_hasnothing", vendorChar.Name.ToString())
                    : mode == ContainerMode.Corpse
                        ? i18n.GetString("inventory_nothingleft")
                        : i18n.GetString("inventory_empty");
                ...
                else
                {
                    ...
                    foreach (var carriedItem in other.Tokens)
                    {
                        ...
                        if (carriedItem.Path("cursed/known") != null)
                            itemString =
                                itemString.Remove(width - 6) +
                                "C"; //DO NOT TRANSLATE -- Curses will be replaced with better terms and variants such as "Slippery" or "Sticky".
                ...
                {
                    Left = 0, Top = 0, Width = Program.Cols - 1, Height = 1, Background = UIColors.StatusBackground,
                    Foreground = UIColors.StatusForeground
                });
                UIManager.Elements.Add(
                    new UILabel(i18n.GetString(mode == ContainerMode.Vendor
                        ? "inventory_pressenter_vendor"
                        : "inventory_pressenter_container"))
                    {
                        Left = 0, Top = 0, Width = Program.Cols - 1, Height = 1, Background = UIColors.StatusBackground,
                        Foreground = UIColors.StatusForeground
                    });
                ...
                {
                    Left = 2, Top = Program.Rows - 10, Width = Program.Cols - 4, Height = 8,
                    Title = UIColors.RegularText
                };
                description = new UILabel(string.Empty) {Width = Program.Cols - 8, Height = 5};
                description.Move(2, 1, descriptionWindow);
                capacity = new UILabel(string.Format("{0:F2}/{1:F2}", player.Character.Carried,
                    player.Character.Capacity));
                ...
                if (mode == ContainerMode.Vendor)
                    capacity.Text = i18n.Format("inventory_money", vendorChar.Name.ToString(),
                        vendorChar.GetToken("money").Value, player.Character.GetToken("money").Value);
                ...
                if (containerList != null)
                {
                    ...
                    containerList.Enter = (s, e) =>
                    {
                        if (containerList.Items.Count == 0)
                            return;
                        //onLeft = true;
                        var tryAttempt = TryRetrieve(player, containerTokens[containerList.Index],
                            containerItems[containerList.Index]);
                        if (tryAttempt.IsBlank())
                        {
                            ...
                            if (containerList.Items.Count == 0)
                            {
                                ...
                                containerList.Height =
                                    (containerList.Items.Count < 10) ? containerList.Items.Count : 10;
                ...
                if (playerList != null)
                {
                    ...
                    playerList.Enter = (s, e) =>
                    {
                        if (playerList.Items.Count == 0)
                            return;
                        //onLeft = false;
                        var tryAttempt = TryStore(player, playerTokens[playerList.Index],
                            playerItems[playerList.Index]);
                        if (tryAttempt.IsBlank())
                        {
        ...
        private static string TryRetrieve(BoardChar boardchar, Token token, InventoryItem chosen)
        {
            ...
            if (token.HasToken("cursed"))
            {
                ...
                return mode == ContainerMode.Vendor
                    ? "It's cursed. " + vendorChar.Name.ToString() + " can't unequip it."
                    : "It's cursed. You shouldn't touch this."; //DO NOT TRANSLATE -- Curses will be replaced with better terms and variants such as "Slippery" or "Sticky".
            }
        ...
        private static string TryStore(BoardChar boardchar, Token token, InventoryItem chosen)
        {
            ...
            if (token.HasToken("cursed"))
            {
                ...
                if (!token.GetToken("cursed").HasToken("known"))
                    token.GetToken("cursed").AddToken("known");
                return
                    "It's cursed! You can't unequip it."; //DO NOT TRANSLATE -- Curses will be replaced with better terms and variants such as "Slippery" or "Sticky".
            }
            ...
            if (mode == ContainerMode.Vendor && token.HasToken("owner") &&
                token.GetToken("owner").Text == vendorChar.Name.ToID())
            {
