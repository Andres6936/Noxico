...
namespace Noxico
{
            ...
            foreach (var set in templateSource.Where(x => x.Name == "set"))
            {
                for (var i = 0; i < set.Tokens.Count; i++) //can't foreach cos we're editing the list as we go :<
                {
                    ...
                    if (template.HasToken("flip-h"))
                    {
                        ...
                        var lines = map.Text.Split(nl, StringSplitOptions.RemoveEmptyEntries).Select(l => l.Trim())
                            .ToArray();
        ...
        public void Create(BiomeData biome, string templateSet)
        {
            ...
            for (var row = 0; row < plotRows; row++)
            {
                for (var col = 0; col < plotCols; col++)
                {
                    ...
                    var newBuilding = new Building(string.Format("house{0}x{1}", row, col), newTemplate, sX, sY,
                        Culture);
                ...
                for (var col = 0; col < plotCols; col++)
                {
                    if (plots[col, row].BaseID == null)
                    {
                        ...
                        if (includeClutter && Random.Flip())
                            Board.AddClutter(col * plotWidth, row * plotHeight, (col * plotWidth) + plotWidth,
                                (row * plotHeight) + plotHeight + row);
                        else
                            safeZones.Add(new Rectangle()
                            {
                                Left = col * plotWidth, Top = row * plotHeight, Right = (col * plotWidth) + plotWidth,
                                Bottom = (row * plotHeight) + plotHeight + row
                            });
                        continue;
                    }
                                        ...
                                        if (m.Text != "tile" && m.Text != "floor" && m.Text != "water")
                                        {
                                            ...
                                            if (m.Text == "bed")
                                            {
                                                var newBed = new Clutter()
                                                {
                                                    ...
                                                    Name = "Bed",
                                                    ID = "Bed_" + (owner == null
                                                        ? Board.Entities.Count.ToString()
                                                        : owner.Name.ToID()),
                                                    Description = owner == null
                                                        ? i18n.GetString("freebed")
                                                        : i18n.Format("someonesbed", owner.Name.ToString(true)),
                                                    ParentBoard = Board,
                                                };
                                            ...
                                            if (m.Text == "container")
                                            {
                                                ...
                                                if (m.HasToken("wardrobe"))
                                                    type = "wardrobe";
                                                var contents = DungeonGenerator.GetRandomLoot("container", type,
                                                    new Dictionary<string, string>()
                                                    {
                                                        {"gender", owner.PreferredGender.ToString().ToLowerInvariant()},
                                                        {
                                                            "biome",
                                                            BiomeData.Biomes[DungeonGenerator.DungeonGeneratorBiome]
                                                ...
                                                var newContainer =
                                                    new Container(type,
                                                            contents) //owner == null ? type.Titlecase() : owner.Name.ToString(true) + "'s " + type, contents)
                                                        {
                                                            XPosition = sX + x,
                                                            YPosition = sY + y,
                                                            ID = "Container_" + type + "_" +
                                                                 (owner == null
                                                                     ? Board.Entities.Count.ToString()
                                                                     : owner.Name.ToID()),
                                                            ParentBoard = Board,
                                            ...
                                            else if (m.Text == "clutter")
                                            {
                                                ...
                                                else
                                                {
                                                    var newClutter = new Clutter()
                                                    {
                                                        ...
                                                        Description = m.HasToken("description")
                                                            ? m.GetToken("description").Text
                                                            : string.Empty,
    ...
    class StoneDungeonGenerator : BaseDungeonGenerator
    {
        ...
        {
            return new Point((int) (rect.Bounds.X + 0.5 * rect.Bounds.Width),
                (int) (rect.Bounds.Y + 0.5 * rect.Bounds.Height));
        }
        ...
        public void Create(BiomeData biome)
        {
            ...
            for (var i = 0; i < MAX_ROOMS; i++)
            {
                ...
                var room = new Room(new SysRectangle(l, t, w, h),
                    (RoomMaterials) Random.Next(Enum.GetValues(typeof(RoomMaterials)).Length));
                var pass = false;
                if (i > 0)
                {
                ...
                if (i > 0)
                {
                    ...
                    if (Random.Flip())
                    {
                        corridors.Add(new Room(
                            new SysRectangle(firstToLast.X, firstToLast.Y, secondToLast.X - firstToLast.X, 1),
                            RoomMaterials.Stone));
                        corridors.Add(new Room(
                            new SysRectangle(secondToLast.X, firstToLast.Y, 1, secondToLast.Y - firstToLast.Y),
                            RoomMaterials.Stone));
                    }
                    else
                    {
                        corridors.Add(new Room(
                            new SysRectangle(firstToLast.X, firstToLast.Y, 1, secondToLast.Y - firstToLast.Y),
                            RoomMaterials.Stone));
                        corridors.Add(new Room(
                            new SysRectangle(firstToLast.X, secondToLast.Y, secondToLast.X - firstToLast.X, 1),
                            RoomMaterials.Stone));
                    }
                }
            }
        ...
        public override void ToTilemap(ref Tile[,] map)
        {
            ...
            foreach (var corridor in corridors)
            {
                ...
                foreach (var point in Toolkit.Line(bounds.Left, bounds.Top, bounds.Right, bounds.Bottom, true))
                {
                    ...
                    if (there != null && there.Definition.Wall && there.Definition.CanBurn && here.Definition.Wall &&
                        !here.Definition.CanBurn)
                    {
        ...
        public override void ToSectorMap(Dictionary<string, Rectangle> sectors)
        {
            for (var row = 0; row < 3; row++)
            {
                for (var col = 0; col < 6; col++)
                {
                    ...
                    sectors.Add(key,
                        new Rectangle()
                            {Left = col * 13, Right = (col * 13) + 13, Top = row * 16, Bottom = (row * 16) + 16});
                }
            }
        }
    }
